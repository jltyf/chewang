...
class Scenario(ScenarioGenerator):
    ...
    def scenario(self, **kwargs):
        ...
        for i in range(len(self.obs)):
            ...
            if self.pedestrian_flag == 1 and self.obs[i][0].ObjectType == 5:
        ...
        for player in self.object_dict:
            ...
            # 使目标轨迹结束后离开视野(非行人)
            if not (self.pedestrian_flag and row[0].ObjectType == 5):
            ...
            if self.pedestrian_flag and row[0].ObjectType == 5:
...
class Task_c:
    ...
    def generateScenarios(self, abs_path):
        ...
        原始自然驾驶场景还原
        '''
        pos_path = os.path.join(abs_path, 'pos.csv')  # 用来生成场景中自车和他车的路径 # 用来生成场景中自车和他车的路径
    ...
    def batchRun(self, path):
        ...
        for di, absPath in enumerate(sorted(files)):
            ...
            # print(absPath)
            self.generateScenarios(absPath)
            # self.generateScenarios_raw(absPath)
            # try:
